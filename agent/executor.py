...
class Executor:
    ...
    def __init__(
        ...
        self.audit = AuditLogger(job_id=job_id, state_dir=state_dir) if (state_dir and job_id) else None
    ...
    def _execute_tool_call(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("denied"):
                ...
                if self.audit:
                    self.audit.log_denial(tool_name, args, result.get("error", "Denied"))
    ...
    async def execute_plan(self, plan: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Starting plan execution with {len(plan)} steps", steps=len(plan))
        
        for i, step in enumerate(plan):
            tool_name = step.get("tool")
            args = step.get("args", {})
            
            self.logger.info(f"Step {i+1}/{len(plan)}: {tool_name}", step=i+1, total=len(plan))
            ...
            if result.get("success"):
                if result.get("denied"):
                    self.logger.warning(f"Action denied: {tool_name}", tool=tool_name, reason=result.get("error"))
                else:
                    self.logger.info(f"Step {i+1} succeeded: {tool_name}", step=i+1, tool=tool_name)
            else:
                self.logger.error(
    ...
    async def execute_with_retry(
        ...
        while iteration < max_iterations:
            ...
            if result["success"]:
                return {"success": True, "iterations": iteration, "results": full_results}
