...
class Metrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get current metrics."""
        avg_job_time = sum(self.job_times) / len(self.job_times) if self.job_times else 0
        avg_tool_time = sum(self.tool_times) / len(self.tool_times) if self.tool_times else 0
